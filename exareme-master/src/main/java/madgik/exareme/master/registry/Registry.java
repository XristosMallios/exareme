...
public class Registry {
    ...
    private Registry(String path) {
        ...
        try {
            ...
            stmt.execute(
                    "create table if not exists sql(" + "table_name text, " + "size integer, " + "sql_definition text, "
                            + "sqlQuery text, " + "isTemporary integer, " + "hashID integer, " + "pin integer, "
                            + "last_access DATETIME, " + "storage_time DATETIME, "
                            + "num_of_access integer, " + "primary key(table_name));");
    ...
    public void addPhysicalTable(PhysicalTable table) {
        ...
        try (
                PreparedStatement insertSqlStatement =
                        regConn.prepareStatement("INSERT INTO sql(table_name, sql_definition, size, isTemporary, " +
                                "sqlQuery, hashID, pin, num_of_access, last_access, storage_time) " +
                                "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

                PreparedStatement insertSqlInfoStatement = regConn.prepareStatement("INSERT INTO sqlInfo(table_name, " +
                        "partition_column) VALUES(?, ?)");


                PreparedStatement insertPartitionStatement = regConn.prepareStatement(
...
//        String psString = "UPDATE sql SET pin = pin+1 " +
    ...
    public Map<String, Integer> getWorkersSize() {
        ...
        try (Statement statement = regConn.createStatement()) {
            ResultSet rs = statement.executeQuery(
                    "SELECT `location`, SUM(partition.`size`) AS `size` FROM partition, sql " +
    ...
    public Map<String, Map<String, Integer>> getSizePerQuery() {
        ...
        try (Statement statement = regConn.createStatement()) {
            ResultSet rs = statement.executeQuery(
                    "SELECT `table_name`, `location`, SUM(`size`) AS `size` FROM partition " +
    ...
    public String containsQuery(String query) {
        ...
        try (Statement statement = regConn.createStatement()) {
            ResultSet rs = statement.executeQuery(
                    "SELECT table_name " + "FROM sql " + "WHERE sqlQuery = '" + query + "';");
    ...
    public String containsHashID(int hashID) {
        ...
        try (Statement statement = regConn.createStatement()) {
            ResultSet rs = statement.executeQuery(
                    "SELECT table_name " + "FROM sql " + "WHERE hashID = '" + hashID + "';");
    ...
    public int getNumOfPartitions(String tableName) {
        ...
        try (Statement statement = regConn.createStatement()) {
            ResultSet rs = statement.executeQuery(
                    "SELECT COUNT(*) FROM partition WHERE table_name='" + tableName + "';");
    ...
    public String getPartitionColumn(String tableName) {
        ...
        try (Statement statement = regConn.createStatement()) {
            ResultSet rs = statement.executeQuery(
                    "SELECT partition_column FROM sqlinfo WHERE table_name='" + tableName + "';");
    ...
    public PhysicalTable getPhysicalTable(String name) {
        ...
        try (
            ...
            if (rs.next()) {
                ...
                table.setHashID(rs.getInt("hashID"));
            ...
            rs = getSqlInfoStatement
                    .executeQuery("SELECT * FROM sqlinfo " + "WHERE table_name = '" + name + "'");
            ...
            rs = getPartitionStatement.executeQuery(
                    "SELECT partition_number, " + "partition_column, location, size " + "FROM partition "
